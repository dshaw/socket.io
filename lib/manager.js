...
Manager.prototype.onHandshake = function (id, data) {
  // caveat: This buries any updates to the handshake data.
  //         For now an acceptable tradeoff, since this gets call 3 times per connection otherwise.
  if (!this.handshaken[id]) {
    this.emit('handshake', id, data);
  }
...
Manager.prototype.handleHandshake = function (data, req, res) {
  var self = this
    ...
    , id = self.generateId()
    , handshakeData = this.handshakeData(data);
  ...
  function writeErr (status, message) {
    ...
    self.emit('handshake', id, handshakeData, message, status );
  };
