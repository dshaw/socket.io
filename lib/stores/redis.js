...
function Redis (opts) {
  ...
  if (opts.redisPub instanceof RedisClient) {
    ...
    if (opts.redisPub.pass) {
      this.pub.auth(opts.redisPub.pass, function (err) {
        if (err) throw err;
      });
    }
    if (opts.redisPub.db) {
      redisClient.select(opts.redisPub.db);
    }
  }
  if (opts.redisSub instanceof RedisClient) {
    ...
    if (opts.redisSub.pass) {
      this.sub.auth(opts.redisSub.pass, function (err) {
        if (err) throw err;
      });
    }
    if (opts.redisSub.db) {
      redisClient.select(opts.redisSub.db);
    }
  }
  if (opts.redisClient instanceof RedisClient) {
    ...
    if (opts.redisClient.pass) {
      this.cmd.auth(opts.redisClient.pass, function (err) {
        if (err) throw err;
      });
    }
    if (opts.redisClient.db) {
      redisClient.select(opts.redisClient.db);
    }
  }
