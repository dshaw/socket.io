...
function Redis (opts) {
  ...
  this.pub.setMaxListeners(20);
  this.sub.setMaxListeners(20);
  this.cmd.setMaxListeners(20);

  console.log('node', this.nodeId);
  this.sub.subscribe('nodes');
  this.publish(this.nodeId);
...
Redis.prototype.subscribe = function (name, consumer, fn) {
  console.log('sub subscribe', name);
      ...
      if (name == ch) {
        ...
        self.on('unsubscribe', function unsubscribe (ch) {
          if (name == ch) {
            ...
            // TODO: Is this supposed to be removeListener  
