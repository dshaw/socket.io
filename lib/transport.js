...
Transport.prototype.handleRequest = function (req) {
  ...
  if (req.method == 'GET') {
    ...
    this.store.publish('open:' + this.id, function () {
      ...
      self.log.debug('Transport handleRequest - paused?', self.paused);
      if (!self.paused) {
        self.log.debug('Transport handleRequest - not paused - subscribing', self.subscribe);
        self.subscribe();
      }
    });
...
Transport.prototype.setHandlers = function () {
  ...
  // TODO: All of these will need to be callable. From the context of the store, not bound.
...
Transport.prototype.subscribe = function () {
  if (!this.subscribed) {
    ...
    this.store.client(this.id).consume(function (payload, packet) {
      self.log.info('store client consume [payload, packet]:', arguments);
//      self.log.warn(self);
//      self.log.error(self.payload);
//      self.log.error(self.write);
      if (payload) {
